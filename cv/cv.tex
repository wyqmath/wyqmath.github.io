...
\begin{document}
	...
	\begin{enumerate}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*, label={[\arabic*]}]
		...
		\item \textbf{Wang, Y.*}, Cai, M., Ma, Y., \& Wei, K. (2026). Sequence-context-aware decoding enables robust reconstruction of protein dynamics from crystallographic B-factors. \textit{Learning Meaningful Representations of Life (LMRL) Workshop at ICLR 2026}.
		\item \textbf{Wang, Y.}, Ma, Y., Chang, Y., et al. (2025). Diffusion Models at the Drug Discovery Frontier: A Review on Generating Small Molecules versus Therapeutic Peptides. \textit{Biology} (JCR Q1, CAS Q2).
		\item Wang, X.†*, \textbf{Wang, Y.†*}, Huang, T. Y., et al. (2025). Octopus Inspired Optimization (OIO): A Hierarchical Framework for Navigating Protein Fitness Landscapes. \textit{In 2025 IEEE International Conference on Bioinformatics and Biomedicine (BIBM)}. IEEE. (CCF-B, Co-first \& co-corresponding author).
		\item \textbf{Wang, Y.*}, \& Huang, T. Y.* (2026). Comment on: "GLM7 – A Novel Composite Glycolipid Index Derived from Routine Health Indicators for Enhanced Diagnosis and Prediction of Multimorbidity". \textit{Advanced Science}. (correspondence, JCR Q1, CAS Q1, First \& corresponding author).
		\item \textbf{Wang, Y.*}, Cai, M., \& Huang, T. Y. (2025). AI for disease prediction: Performance insights and key limitations. \textit{Journal of Clinical Neuroscience, 138}, 111360. (correspondence, JCR Q3, CAS Q4, First \& corresponding author)
	\end{enumerate}
	...
	\begin{enumerate}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*, label={[\arabic*]}, resume]
		\item \textbf{Wang, Y.*} (2026). Spectral entropy of the discrete Hasimoto effective potential exposes sub-residue geometric transitions in protein secondary structure. \textit{arXiv preprint arXiv:2602.21787}. (Sole \& corresponding author).
		\item \textbf{Wang, Y.*} (2026). Piecewise integrability of the discrete Hasimoto map for analytic prediction and design of helical peptides. \textit{arXiv preprint arXiv:2602.16255}. (Sole \& corresponding author).
		\item \textbf{Wang, Y.*} (2026). Structural barriers of the discrete Hasimoto map applied to protein backbone geometry. \textit{arXiv preprint arXiv:2602.13160}. (Sole \& corresponding author).
	\end{enumerate}
	...
	\begin{enumerate}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*, label={[\arabic*]}, resume]
		\item Wang, X., \textbf{Wang, Y.}, \& Huang, T. Y. (2025). Crypto-ncRNA: Non-coding RNA (ncRNA) Based Encryption Algorithm. \textit{ICLR 2025 Workshop on AI for Nucleic Acids}. (Co-first author).
		\item \textbf{Wang, Y.*}, Chang, Y., Zhang, J., Cai, M., Yan, J., Zhao, H.*, \& Alzheimer's Disease Neuroimaging Initiative. (2026). Geometric Fragility in Alzheimer's Disease: Probing the Loss of Hippocampal Hierarchical Abstraction via Contrastive Point Cloud Modeling. \textit{Proceedings of the Annual Meeting of the Cognitive Science Society (CogSci 2026)}; also presented at \textit{LMRL Workshop at ICLR 2026}. (First \& corresponding author).
		\item \textbf{Wang, Y.}, Liu, Z., Zai, J., Chen, J., \& Sabir, E. (2026). Resilient AI Infrastructure by Design: A Spatially-Aware Framework for Tolerating Clustered Failures. \textit{In 4th Annual AAAI Workshop on AI to Accelerate Science and Engineering (AI2ASE)}.
		\item \textbf{Wang, Y.*}, Huang, T. Y., Gao, Q., Chang, Y., \& Zhang, J. (2026). AI Driven Discovery of Bio Ecological Mediation in Cascading Heatwave Risks. \textit{ICLR 2026 Workshop on Foundation Models for Science: Real-World Impact and Science-First Design (FM4Science)}. (First \& corresponding author).
		\item \textbf{Wang, Y.*}, Cai, M., Zhang, J., Chang, Y., \& Yan, J. (2026). Adaptive Decision-Making in Multi-Stage Production: A Framework for Cost Optimization under Sampling Uncertainty. \textit{Applied Operations and Analytics}.
		\item Zhao, H., Shi, W., Liu, C., \& \textbf{Wang, Y.} (2025). Boundary Box-Guided Targeted Adversarial Attacks with Semantic Perturbation. \textit{In 2025 IEEE International Conference on Systems, Man, and Cybernetics (SMC)} (pp. 1604-1609). IEEE. (CCF-C, EI).
		\item Zuo, J., \textbf{Wang, Y.}, \& Yibulayin, X. (2026). Relative-to-human Benchmark Cognitive Divergence and Semantic Comprehensibility in Chinese–Uyghur LLM Translation. \textit{Frontiers in Psychology}.
		\item \textbf{Wang, Y.*}, Zhang, J., \& Chang, Y. (2024). A probability prediction model for flood disasters based on Multi-layer Perceptron. \textit{Journal of Physics: Conference Series} (Vol. 2905, No. 1, p. 012003). IOP Publishing.
	\end{enumerate}
	...
	\textbf{Chinese Academy of Sciences Innovation Practice Training Program} \hfill 2025.11 – 2026.11
	\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*]
	...
	\textbf{Urumqi Sandaoban Town Cultural Exhibition Project} \hfill 2026.01 – 2026.06
	\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*]
		\item \textbf{Project:} Forging a Strong Sense of Community for the Chinese Nation Exhibition Project (Participant).
	\end{itemize}
	...
	\end{center}
	\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*]
		\item \textbf{UN Global Leadership and ESG Programme $\times$ X-Institute Shenzhen "AI for the Young and the Elderly" Challenge}: Best Innovation Award \hfill 2026
	...
	\begin{multicols}{2}
		\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*]
			\item ICLR 2026 Workshop LMRL
			\item ICLR 2026 Workshop FM4Science
			\item ICML 2026 AI4Science Workshop
	...
	\end{center}
	\textbf{Technical Skills:}Python, C/C++, MATLAB, LaTeX, Linux, HTML/CSS/JavaScript, Single-cell Data Analysis, Multi-omics (Genomics, Proteomics, Transcriptomics), Homology Modeling, Molecular Dynamics Simulation

	\textbf{Research Interests:}Artificial Intelligence, Deep Learning, AI for Science, Bioinformatics, Computational Biology, Representation Learning, Protein Folding, Mathematical Physics (Nonlinear Schr\"{o}dinger Equation \& Protein Folding), Mathematical Modeling, Neuroscience
